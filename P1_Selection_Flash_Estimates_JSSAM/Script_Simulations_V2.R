...
COS <- function(Y,S,Z){
  ...
  for (t in 1:t_total){
    ...
    if (t==1){
      ...
      r_ys_hat <- cor(Y[,t-1],S[,t-1])
      r_zs_hat <- cor(Z[,t-1],S[,t-1])
      S_COS[t] <- (r_ys_hat/r_zs_hat)*(mean(z[,t], na.rm = TRUE) - mean(Z[,t])) 
    }
  }
  return(S_COS)
}
...
cor_aux <- c(0.05, 0.2, 0.4, 0.6, 0.8, 0.99) # These are the correlations that we will use in the analyses
...
names(Z_betas) <- paste0("Z_b",1:nrow(alpha_beta)) # If we name the list within the list it will be 'easier' to access them
...
for (b in 1:length(betas)){
  ...
  for (c in 1:length(cor_sel)){
    S_betas[[b]][[c]] <- corS(betas[[b]],cor_sel[c])
...
# 1. Normal distribution
SelM_hat <- Sel <- Meng_rYS_hat <- array(data = NA, dim = c(Time, length(cor_sel), D))
Perf <- as.data.frame(matrix(data = NA, nrow = length(cor_sel), ncol = (2*D)))
  
for (d in 1:D){
  for (c in 1:length(cor_sel)){
   Meng_rYS_hat[,c,d] <- Meng_rYS(as.matrix(Y[,,d]), as.matrix(S[[c]][,,d]))
   Perf[c,(d*2)-1] <- MAD(as.matrix(Y[,,d]), as.matrix(S[[c]][,,d]), Meng_rYS_hat[,c,d])[[3]]
   Perf[c,(d*2)] <- RMSD(as.matrix(Y[,,d]), as.matrix(S[[c]][,,d]), Meng_rYS_hat[,c,d])[[3]]
   Sel[,c,d] <- as.vector(MAD(as.matrix(Y[,,d]), as.matrix(S[[c]][,,d]), Meng_rYS_hat[,c,d])[[1]])
   SelM_hat[,c,d] <- as.vector(MAD(as.matrix(Y[,,d]), as.matrix(S[[c]][,,d]), Meng_rYS_hat[,c,d])[[2]])
  }
}
    
Perf <- as.data.frame(cbind(as.vector(paste0("S",1:4)), 
                            as.vector(rep("a. Meng_rYS",4)),
                            as.vector(rep("Y",4)),
                            as.vector(rep(NA,4)), 
                            Perf))
...
for (b in 1:length(betas)){
  ...
  for (d in 1:D){
   for (c in 1:length(cor_sel)){
     Meng_rYS_hat[,c,d] <- Meng_rYS(as.matrix(betas[[b]][,,d]), as.matrix(S_betas[[b]][[c]][,,d]))
     Perf[c,(d*2)-1] <- MAD(as.matrix(betas[[b]][,,d]), as.matrix(S_betas[[b]][[c]][,,d]), Meng_rYS_hat[,c,d])[[3]]
     Perf[c,(d*2)] <- RMSD(as.matrix(betas[[b]][,,d]), as.matrix(S_betas[[b]][[c]][,,d]), Meng_rYS_hat[,c,d])[[3]]
   }
  }
    
  Perf <- as.data.frame(cbind(as.vector(paste0("S",1:length(cor_sel))),
                              as.vector(rep("a. Meng_rYS",length(cor_sel))),
                              as.vector(rep(paste0("beta",b),length(cor_sel))), 
                              as.vector(rep(NA,length(cor_sel))), 
                              Perf))
  names <- expand.grid(c("MAD_D","RMSD_D"),1:D)
  colnames(Perf) <-  c(c("S","M","Y","Z"), as.character(paste0(names$Var1,names$Var2)))
...
# 1. Normal distribution
Sel_hat <- Meas_hat <- vector(mode = "list", length = length(Measures))
Perf_M <- vector(mode = "list", length = length(Measures))
    
for (cz in 1:length(cor_aux)){
  for (m in 1:length(Measures)){
    Sel_hat[[m]] <- Meas_hat[[m]] <- array(data = NA, dim = c(Time, length(cor_sel), D))
    Perf_M[[m]] <- as.data.frame(matrix(data = NA, nrow = length(cor_sel), ncol = (2*D)))
    for (d in 1:D){
     for (cs in 1:length(cor_sel)){
      Meas_hat[[m]][,cs,d] <- get(Measures[m])(as.matrix(Y[,,d]), as.matrix(S[[cs]][,,d]), Z[[cz]][,,d])
      Perf_M[[m]][cs,(d*2)-1] <- MAD(as.matrix(Y[,,d]), as.matrix(S[[cs]][,,d]), Meas_hat[[m]][,cs,d])[[3]]
      Perf_M[[m]][cs,(d*2)] <- RMSD(as.matrix(Y[,,d]), as.matrix(S[[cs]][,,d]), Meas_hat[[m]][,cs,d])[[3]]
      Sel_hat[[m]][,cs,d] <- as.vector(MAD(as.matrix(Y[,,d]), as.matrix(S[[cs]][,,d]), Meas_hat[[m]][,cs,d])[[2]])
     }
    }
    
    Perf_M[[m]] <- as.data.frame(cbind(as.vector(paste0("S", 1:length(cor_sel))), 
                                       as.vector(rep(Measures[m],length(cor_sel))), 
                                       as.vector(rep("Y", length(cor_sel))),
                                       as.vector(rep(paste0("Z",cz),length(cor_sel))), 
                                       as.matrix(Perf_M[[m]])))
...
for (b in 1:length(betas)){
  ...
  for (cz in 1:length(cor_aux)){
    for (m in 1:length(Measures)){
      ...
      for (d in 1:D){
        for (cs in 1:length(cor_sel)){
          Meas_hat[[m]][,cs,d] <- get(Measures[m])(as.matrix(betas[[b]][,,d]), as.matrix(S_betas[[b]][[cs]][,,d]), Z_betas[[b]][[cz]][,,d])
          Perf_M[[m]][cs,(d*2)-1] <- MAD(as.matrix(betas[[b]][,,d]), as.matrix(S_betas[[b]][[cs]][,,d]), Meas_hat[[m]][,cs,d])[[3]]
          Perf_M[[m]][cs,(d*2)] <- RMSD(as.matrix(betas[[b]][,,d]), as.matrix(S_betas[[b]][[cs]][,,d]), Meas_hat[[m]][,cs,d])[[3]]
        }
      }
...
data <- mPerformance[which(mPerformance$Z=="Z4" | is.na(mPerformance$Z)),]
...
Plot_S_MAD <- ggplot(data = data[grep("MAD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,4) +  ylab("Estimator") +
...
Plot_S_RMSD <- ggplot(data = data[grep("RMSD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,4) + ylab("Estimator") +
...
data <- data[grep("Y",data$Y),]
data$value <- as.numeric(data$value)
data$Z[data$Z=="Z2"] <- "Cov(YZ) = 0.2"
data$Z[data$Z=="Z3"] <- "Cov(YZ) = 0.4"
data$Z[data$Z=="Z4"] <- "Cov(YZ) = 0.6"
data$Z[data$Z=="Z5"] <- "Cov(YZ) = 0.8"
data <- data[!is.na(data$Z),]
data <- data[(!data$Z=="Z1" & !data$Z=="Z6"),]

Plots_Z_MAD <- ggplot(data = data[grep("MAD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,5) +  ylab("Estimator") +
  scale_fill_viridis_c(name = "MAD", begin = 0, end = 1, option = "D") +
  theme(text = element_text(size = 20)) + xlab("MAD") + facet_grid(~Z)
Plots_Z_MAD
ggsave("Plot_Z_MAD.png", path = fold_graphs, width = 40, height = 15, units = "cm")

Plots_Z_RMSD <- ggplot(data = data[grep("RMSD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,5) +  ylab("Estimator") +
  scale_fill_viridis_c(name = "RMSD", begin = 0, end = 1, option = "D") +
...
data <- data[grep("Y",data$Y),]
data$value <- as.numeric(data$value)
data$Z[data$Z=="Z1"] <- "Cov(YZ) = 0.05"
data$Z[data$Z=="Z3"] <- "Cov(YZ) = 0.4"
data$Z[data$Z=="Z4"] <- "Cov(YZ) = 0.6"
data$Z[data$Z=="Z6"] <- "Cov(YZ) = 0.95"
data <- data[!is.na(data$Z),]
data <- data[(!data$Z=="Z2" & !data$Z=="Z5"),]

Plots_Z_MAD <- ggplot(data = data[grep("MAD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,20) +  ylab("Estimator") +
  scale_fill_viridis_c(name = "MAD", begin = 0, end = 1, option = "D") +
  theme(text = element_text(size = 20)) + xlab("MAD") + facet_grid(~Z)
Plots_Z_MAD
ggsave("Plot_Z_MAD_ext.png", path = fold_graphs, width = 40, height = 15, units = "cm")
...
Plots_Z_RMSD <- ggplot(data = data[grep("RMSD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  ...
  theme(text = element_text(size = 20)) + xlab("RMSD") + facet_grid(~Z)
Plots_Z_RMSD
ggsave("Plot_Z_RMSD_ext.png", path = fold_graphs, width = 40, height = 15, units = "cm")

# (iii) Different Y, ceteris paribus - To check how the estimators react to changes in Y distribution
  
for (b in 1:length(betas)){
  data <- mPerformance[which(mPerformance$Z=="Z3" | is.na(mPerformance$Z)),]
...
Plot_B_MAD <- ggplot(data = data[grep("MAD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,4) + ylab("Estimator") +
...
Plot_B_RMSD <- ggplot(data = data[grep("RMSD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) + 
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,1) + ylab("Estimator") +
...
for (b in 1:length(betas)){
  data <- mPerformance[which(mPerformance$Z=="Z2" | is.na(mPerformance$Z)),]
  data$Alpha <-  data$Beta <- NA
  for (i in 1:4){
    data$Alpha[grep(paste0("beta",i),data$Y)] <- data$Alpha[grep(paste0("beta",i+4),data$Y)] <- data$Alpha[grep(paste0("beta",i+8),data$Y)] <- data$Alpha[grep(paste0("beta",i+12),data$Y)] <- i
    data$Beta[grep(paste0("beta",i),data$Y)] <- 1
...
Plot_BS_MAD <- ggplot(data = data[grep("MAD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,4) + ylab("Estimator") +
...
Plot_BS_RMSD <- ggplot(data = data[grep("RMSD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,2) + ylab("Estimator") +
...
Plot_BZ_MAD <- ggplot(data = data[grep("MAD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) + xlim(0,0.5) + ylab("Estimator") +
  scale_fill_viridis_c(name = "MAD", option = "B") + theme(text = element_text(size = 25)) +
  xlab("MAD") + facet_grid(Z~Betas)
Plot_BZ_MAD
ggsave("Plot_BZ_MAD.png", path = fold_graphs, width = 50, height = 50, units = "cm")

Plot_BZ_RMSD <- ggplot(data = data[grep("RMSD",data$variable),], aes(x = value, y = M, fill = after_stat(x))) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01)  + xlim(0,0.5) + ylab("Estimator") +
  scale_fill_viridis_c(name = "RMSD", option = "B") + theme(text = element_text(size = 25)) +
...
# What is the sample mean and the median of these estimates

Perf_Av <- aggregate(mPerformance$value, data = mPerformance[grep("MAD",mPerformance$variable),], by = list(mPerformance$S, mPerformance$M, mPerformance$Z), mean, na.action = NULL, na.rm = TRUE)
Perf_Av <- Perf_Av[Perf_Av$Group.1=="S3",]
Perf_Av <- reshape(data = Perf_Av, idvar = "Group.2", v.names = "x", timevar = c("Group.3"), direction = "wide", drop = "Group.1")
colnames(Perf_Av) <- c("Estimator", "Cov(YZ)=0.05", "Cov(YZ)=0.2", "Cov(YZ)=0.4", "Cov(YZ)=0.6", "Cov(YZ)=0.8", "Cov(YZ)=0.95")
Tab_Av <- kable(Perf_Av, format = "latex")
setwd(fold_graphs)
save_kable(Tab_Av, file = "Table_Means_Z.pdf")

Perf_Med <- aggregate(mPerformance$value, data = mPerformance[grep("MAD",mPerformance$variable),], by = list(mPerformance$S, mPerformance$M, mPerformance$Z), median, na.action = NULL, na.rm = TRUE)
Perf_Med <- Perf_Med[Perf_Med$Group.1=="S3",]
Perf_Med <- reshape(data = Perf_Med, idvar = "Group.2", v.names = "x", timevar = c("Group.3"), direction = "wide", drop = "Group.1")
colnames(Perf_Med) <- c("Estimator", "Cov(YZ)=0.05", "Cov(YZ)=0.2", "Cov(YZ)=0.4", "Cov(YZ)=0.6", "Cov(YZ)=0.8", "Cov(YZ)=0.95")
Tab_Med <- kable(Perf_Med, format = "latex")
save_kable(Tab_Med, file = "Table_Medians_Z.pdf")

# Graphs to check the selection

# True Selection
AvSel_M <- matrix(data = NA, ncol = D, nrow = length(cor_sel))
AvSel_V <- numeric(length(cor_sel))

for (s in 1:length(cor_sel)){
  for (d in 1:D){
    AvSel_M[s,d] <- mean(abs(Sel[,s,d]))   
  }
  AvSel_V[s] <- mean(AvSel_M[s,]) 
}

AvSel <- as.data.frame(cbind(paste0("S",1:4),AvSel_V))
colnames(AvSel) <- c("S", "Sel")
AvSel <- AvSel %>%
  mutate(S = recode(S, "S1" = "Cov(YS)=0.2", "S2" = "Cov(YS)=0.4", "S3" = "Cov(YS)=0.6", "S4" = "Cov(YS)=0.8 "))

# Estimated Selection
Sel_hat <- append(list(SelM_hat), Sel_hat)
AvSelh_A <- array(data = NA, dim = c(length(Sel_hat), length(cor_sel), D))
AvSelh_M <- matrix(data = NA, nrow = length(Sel_hat), ncol = length(cor_sel))

for (i in 1:length(Sel_hat)){
  for (s in 1:length(cor_sel)){
    for (d in 1:D){
      AvSelh_A[i,s,d] <- mean(abs(Sel_hat[[i]][,s,d]), na.rm = TRUE)   
    }
    AvSelh_M[i,s] <- mean(AvSelh_A[i,s,]) 
  }
}

AvSelh_M <- cbind(c("Meng_rYS", Measures), AvSelh_M)

colnames(AvSelh_M) <- c("M", paste0("S",1:length(cor_sel)))
Dat_Sel <- AvSelh_M %>%
  as_tibble() %>%
  gather(S, Sel, S1:S4) %>%
  mutate(S = recode(S, "S1" = "Cov(YS)=0.2", "S2" = "Cov(YS)=0.4", "S3" = "Cov(YS)=0.6", "S4" = "Cov(YS)=0.8 "))

ggplot(data = Dat_Sel, aes(x = M, y = as.numeric(Sel), fill = M)) + 
  geom_col() +
  facet_grid(~S) +
  coord_flip() +
...
ggsave("Plot_Sel.png", path = fold_graphs, width = 40, height = 15, units = "cm")
