...
# 1. DDP(h): Y_n - mu_{Y} = Cov(Y_{t-1}, S_{t)

DDP_l <- function(Y, S, Z){
  P <- ncol(Y)               # No. of periods
  N <- apply(Y, 2, length)   # Population size
  n <- apply(S, 2, sum)      # Sample size
  ys <- Y*S; ys <- ifelse(S==1, Y, NA) # Y in the sample
  # True values
  D_I <- r_ys <- mapply(cor, as.data.frame(Y), as.data.frame(S))
  D_U <- sigma_y <- apply(Y, 2, p.sd)
  D_O <- sqrt((N-n)/n)
  # Get the info from the lags
  Yl <- (cbind(rep(NA, times = nrow(Y)), Y[,-P]))
  Sl <- (cbind(rep(NA, times = nrow(Y)), S[,-P]))
  Nl <- apply(Yl, 2, length)   # Population size
  nl <- apply(Sl, 2, sum)      # Sample size
  ysl <- ifelse(Sl == 1, Yl, NA)
  # Estimates
  D_I_hat <- r_ys_hat <- mapply(cor, as.data.frame(Yl), as.data.frame(S))
  D_U_hat <- sigma_y_hat <- apply(Yl, 2, p.sd)
  S_DDP <- D_I_hat*D_U_hat*D_O
  res <- list(s_DDP = S_DDP, D_I = D_I, D_U = D_U, D_O = D_O, r_ys = r_ys)
  #sigma_ysl <- mapply(cov, as.data.frame(Yl), as.data.frame(S))/(n/N)
  return(res)
}
...
MUB_l <- function(Y, S, Z){
  ...
  r_yz <- mapply(cor, as.data.frame(Y), as.data.frame(Z))
  ...
  Sl <- (cbind(rep(NA, times = nrow(S)), S[,-P]))
  ysl <- ifelse(Sl==1, Yl, NA)
  zsl <- ifelse(Sl==1, Zl, NA)
  dyl <- (colMeans(ysl, na.rm = T) - colMeans(Yl))/colSds(Yl)
  dzl <- (colMeans(zsl, na.rm = T) - colMeans(Zl))/colSds(Zl)
  r_yzl_hat <- mapply(cor, as.data.frame(Yl), as.data.frame(Zl))
  r_zsl_hat <- mapply(cor, as.data.frame(Zl), as.data.frame(Sl))
  phi_hat <- (dyl - r_yzl_hat*dzl)/((dyl - r_yzl_hat*dzl) + (dzl - r_yzl_hat*dyl))
  g_hat <- (phi_hat + (1-phi_hat)*r_yzl_hat)/(phi_hat*r_yzl_hat + (1-phi_hat))
  sigma_y_hat <- apply(Yl, 2, p.sd)
  c_obs <- sigma_y_hat/colSds(Zl)
  S_MUB <- g_hat*c_obs*(colMeans(zsl, na.rm = T) - colMeans(Zl))
  r_ys_hat <- g_hat*r_zsl_hat
...
# 3. DDP(a): Y_n - mu_{Y} = Cov(Z_{t}, S_{t})

DDP_c <- function(Y, S, Z){
  P <- ncol(Y)               # No. of periods
  N <- apply(Y, 2, length)   # Population size
  n <- apply(S, 2, sum)      # Sample size
  ys <- ifelse(S==1, Y, NA) # Y in the sample
  # True values
  D_I <- r_ys <- mapply(cor, as.data.frame(Y), as.data.frame(S))
  D_U <- sigma_y <- apply(Y, 2, p.sd)
  D_O <- sqrt((N-n)/n)
  # Estimates with the current aux
  D_I_hat <- r_ys_hat <- mapply(cor, as.data.frame(Z), as.data.frame(S))
  D_U_hat <- sigma_y_hat <- apply(Z, 2, p.sd)
  S_DDP <- D_I_hat*D_U_hat*D_O
  res <- list(S_DDP=S_DDP, D_I=D_I, D_U=D_U, D_O=D_O, r_ys=r_ys)
  #sigma_zs <- mapply(cov, as.data.frame(Z), as.data.frame(S))/(n/N)
  return(res)
}
...
DDP_m <- function(Y, S, Z){
  P <- ncol(Y)               # No. of periods
  N <- apply(Y, 2, length)   # Population size
  n <- apply(S, 2, sum)      # Sample size
  ys <- ifelse(S==1, Y, NA)  # Y in the sample
  zs <- ifelse(S==1, Z, NA)  # Z in the sample
  # True values
  D_I <- r_ys <- mapply(cor, as.data.frame(Y), as.data.frame(S))
  D_U <- sigma_y <- apply(Y, 2, p.sd)
  D_O <- sqrt((N-n)/n)
  # Get the info from the lags
  Yl <- (cbind(rep(NA, times = nrow(Y)), Y[,-P]))
  Zl <- (cbind(rep(NA, times = nrow(Y)), Z[,-P]))
  Sl <- (cbind(rep(NA, times = nrow(Y)), S[,-P]))
  Nl <- apply(Yl, 2, length)   # Population size
  nl <- apply(Sl, 2, sum)      # Sample size
  ysl <- ifelse(Sl == 1, Yl, NA)
  r_zs <- mapply(cor, as.data.frame(Zl), as.data.frame(Sl))
  sigma_z <- apply(Zl, 2, p.sd)
  # Estimates
  D_I_hat <- r_ys_hat <- mapply(cor, as.data.frame(Yl), as.data.frame(Sl))
  D_U_hat <- sigma_y_hat <- apply(Yl, 2, p.sd)
  D_O_hat <-  (colMeans(zs, na.rm = T) - colMeans(Z))/(r_zs*sigma_z)
  S_DDP <- D_I_hat*D_U_hat*D_O_hat
  res <- list(S_DDP = S_DDP, D_I = D_I, D_U = D_U, D_O = D_O, r_ys = r_ys)
  #sigma_ysl <- mapply(cov, as.data.frame(Yl), as.data.frame(S))/(n/N)
  return(res)
}
...
MUB_m <- function(Y, S, Z){
  ...
  dy <- (colMeans(ys, na.rm = T) - colMeans(Y))/colSds(Y)
  dz <- (colMeans(zs, na.rm = T) - colMeans(Z))/colSds(Z)
  r_yz_hat <- mapply(cor, as.data.frame(Yl), as.data.frame(Z))
  r_zs <- mapply(cor, as.data.frame(Z), as.data.frame(S))
...
psi <- runif(n = D, min = 0.4, max = 0.8)  
for (p in 2:Time){
  Y[,p,] <- const + sweep(Y[,p-1,], 2, psi, FUN="*") + e[,p,]
}
...
# 2. Beta distribution
ab <- expand.grid(alpha = 1:4, beta = 1:4)
n_betas <- nrow(ab)
...
for (b in 1:n_betas) {
  ...
  # Initial values from Beta(alpha, beta)
  bt[[b]][,1,] <- bt0[,b,] <- matrix(rbeta(N * D, shape1 = ab$alpha[b], shape2 = ab$beta[b]),
                                     nrow = N, ncol = D)
  Z_t <- qlogis(bt[[b]][,1,])  # Logit transform
  
  for (t in 2:Time) {
    epsilon_t <- rnorm(N, mean = 0, sd = 0.1)
    Z_t <- sweep(Z_t, 2, psi, FUN = "*") + epsilon_t
    bt[[b]][,t,] <- plogis(Z_t)  # Back to [0,1]
  }
}
...
Est <- c("DDP_l", "MUB_l","DDP_c","MUB_c", "DDP_m", "MUB_m")
mres <- c("MAD","RMSD")
perf_y <- expand.grid("S" = 1:4, "Z" = 1:6, "M"= Est, "Y"= "Y", "beta" = NA, "a" = NA, "b" = NA, "res" = mres, "d" = 1:D)
perf_b <- expand.grid("S" = 1:4, "Z" = 1:6, "M"= Est, "Y"= "Beta","beta" = 1:n_betas, "res" = c("MAD", "RMSD"), "d" = 1:D)
...
Est2 <- c("DDP(L)", "MUB(C)","DDP(L)","MUB(C)", "DDP(M)", "MUB(M)")
...
rs_b <- expand.grid("S" = 1:4, "Z" = 1:6, "Y"= "Beta","beta" = 1:n_betas, "d" = 1:D)
...
for (i in 1:length(mres)){
  ...
  p_sz <- perf_plot %>% 
    ...
    geom_boxplot(alpha = 0.5) + scale_fill_manual(values = paired_palette) +
  ...
  p_bs <- perf_plot %>% 
    filter(Z==0.8 & !is.na(a) & grepl(mres[i], res)) %>% 
    ggplot(aes(x = value, y = fct_rev(Estimator), fill = Estimator)) + 
    geom_boxplot(alpha = 0.5) + scale_fill_manual(values = paired_palette) +
    ggh4x::facet_grid2(S~b, scales = "free", independent = "all", labeller = label_bquote(rows = rho[YS]:.(S), col = beta:.(b))) + ylab("") + xlab(mres[i]) +
    theme(text = element_text(size = 30), axis.text.y = element_blank(), axis.text.x = element_text(size = 15),
        axis.ticks.y = element_blank(), legend.key.size = unit(1.5, "cm"))
  ...
  p_bz <- perf_plot %>% 
    filter(S==0.4 & Z!=0.05 & Z!=0.95 & !is.na(a) & grepl(mres[i], res)) %>% 
    ggplot(aes(x = value, y = fct_rev(Estimator), fill = Estimator)) + 
    geom_boxplot(alpha = 0.5) + scale_fill_manual(values = paired_palette) +
    ggh4x::facet_grid2(Z~b, scales = "free", independent = "all", labeller = label_bquote(rows = rho[YZ]:.(Z), col = beta:.(b))) + ylab("") + xlab(mres[i])  +
    theme(text = element_text(size = 30), axis.text.y = element_blank(), axis.text.x = element_text(size = 15),
          axis.ticks.y = element_blank(), legend.key.size = unit(1.5, "cm"))
  ...
  p_b13_sz <- perf_plot %>% 
    filter(Z!=0.05 & Z!=0.95 & !is.na(a) & grepl(mres[i], res) & beta==4) %>% 
    ggplot(aes(x = value, y = fct_rev(Estimator), fill = Estimator)) + 
    geom_boxplot(alpha = 0.5) + scale_fill_manual(values = paired_palette) +
    ggh4x::facet_grid2(Z~S, scales = "free", independent = "all", labeller = label_bquote(rows = rho[YZ]:.(Z), col = rho[YS]:.(S))) + ylab("") + xlab(mres[i])  +
    theme(text = element_text(size = 30), axis.text.y = element_blank(), axis.text.x = element_text(size = 15),
...
arsh %>% 
  ...
  geom_col(alpha = 0.5) +
  geom_errorbar(aes(ymin = rs_h - 1.96*se_rs_h, ymax = rs_h + 1.96*se_rs_h), width = 0.5) +
...
for (i in 1:length(mres)){
  p_sp <- perf_plot %>% 
    ...
    facet_grid(Z~S, labeller = label_bquote(rows = rho[YZ]:.(Z), col = rho[YS]:.(S))) + scale_color_manual(values = paired_palette) +
    theme(text = element_text(size = 30), strip.background = element_blank(), strip.placement = "outside", legend.key.size = unit(1.5, 'cm')) 
...
p_res <- merge(MAD_d[,c("S", "Z", "Estimator", "d", "value")], RMSD_d[,c("S", "Z", "Estimator", "d", "value")], by = c("S", "Z", "Estimator", "d"), suffixes = c("_MAD", "_RMSD")) %>% 
  ...
  geom_point(size = 3, alpha = 0.5) + ylab("RMSD") + xlab("MAD") + scale_color_manual(values = paired_palette) +
  facet_grid(Z~S, labeller = label_bquote(rows = rho[YZ]:.(Z), col = rho[YS]:.(S))) + 
