...
Est <- c("Meng_rYS","Meng_rZS1","Meng_rZS2", "MUB_half", "MUB_hist", "MUB_robust", "MUB_alt", "MUB_abs", "MUB_alt_abs", "COS", "Zdiff")
n_Est <- length(Est)
...
mPerformance$M <- mPerformance$M %>%
  recode("Meng_rYS" = "a. Meng_rYS", "Meng_rZS1" = "b. Meng_rZS1", "Meng_rZS2" = "c. Meng_rZS2",
         "MUB_half" = "d. MUB_half", "MUB_hist" = "e. MUB_hist", "MUB_robust" = "f. MUB_robust",
         "MUB_alt" = "g. MUB_alt", "MUB_abs" = "h. MUB_abs", "MUB_alt_abs" = "i. MUB_alt_abs",
         "COS" = "j. COS", "Zdiff" = "k. Zdiff")
...
for (i in 1:length(Prf_M)){
  ...
  Plot_S <- mPerformance %>%
    filter((Z == "Cor(YZ) = 0.4" | is.na(Z)) & grepl("Y", Y) & grepl(Prf_M[i], variable)) %>% # Selecting only the results that we want to plot 
    ggplot(aes(x = value, y = M, fill = M)) +                                                 # General aesthetic
    geom_boxplot() + scale_fill_dutchmasters(palette="view_of_Delft") +                       # The type of plot that we will use
  ...
  Plot_Z <- mPerformance %>%
    filter(S == "Cor(YS) = 0.6" & grepl("Y", Y) & grepl(Prf_M[i], variable) & Z != "Cor(YZ) = 0.05"  & Z!= "Cor(YZ) = 0.95") %>% # Selecting only the results that we want to plot
    ggplot(aes(x = value, y = M, fill = M)) +                                                                                    # General aesthetic
    geom_boxplot() + scale_fill_dutchmasters(palette="milkmaid") +                                                               # The type of plot that we will use
  ...
  Plot_Z <- mPerformance %>%
    filter(S == "Cor(YS) = 0.6" & grepl("Y", Y) & grepl(Prf_M[i], variable) & Z != "Cor(YZ) = 0.2" & Z!= "Cor(YZ) = 0.8") %>%  # Selecting only the results that we want to plot
    ggplot(aes(x = value, y = M, fill = M)) +                                                                                  # General aesthetic
    geom_boxplot() + scale_fill_dutchmasters(palette="milkmaid") +                                                             # The type of plot that we will use
    facet_grid(~Z) + xlim(0,5) +                                                                                               # Make several panels with specific limits
    ylab("Estimator") + xlab(Prf_M[i])  +                                                                                      # The labels of each axis
  ...
  Plot_B <- mPerformance %>%
    filter((Z == "Cor(YZ) = 0.6" | is.na(Z)) & S == "Cor(YS) = 0.4" & !is.na(Alpha) & grepl(Prf_M[i], variable)) %>% # Selecting only the results that we want to plot
    ggplot(aes(x = value, y = M, fill = M)) +                                                                        # General aesthetic
    geom_boxplot() + scale_fill_dutchmasters(palette="milkmaid") +                                                   # The type of plot that we will use
    facet_grid(Beta~Alpha, labeller = label_both) + xlim(0,0.75) +                                                      # Make several panels with specific limits
    ylab("Estimator") + xlab(Prf_M[i]) +                                                                             # The labels of each axis
    theme(text = element_text(size = 20),
          axis.text.y=element_blank(),
          axis.ticks.y=element_blank())                                                                              # General layout options: Used to adjust the size of the text
  ...
  Plot_BS <- mPerformance %>%
    filter((Z == "Cor(YZ) = 0.6" | is.na(Z)) & !is.na(Alpha) & grepl(Prf_M[i], variable) & Beta == 1) %>% # Selecting only the results that we want to plot
    ggplot(aes(x = value, y = M, fill = M)) +                                                             # General aesthetic
    geom_boxplot() + scale_fill_dutchmasters(palette="milkmaid") +                                        # The type of plot that we will use
    facet_grid(S~Alpha, labeller = label_both) + xlim(0,0.75) +                                              # Make several panels with specific limits
    ylab("Estimator") + xlab(Prf_M[i]) +                                                                  # The labels of each axis
    theme(text = element_text(size = 20),
          axis.text.y=element_blank(),
          axis.ticks.y=element_blank())                                                                   # General layout options: Used to adjust the size of the text
  ...
  Plot_BZ <- mPerformance %>%
    ...
    ggplot(aes(x = value, y = M, fill = M)) +                                                # General aesthetic
    geom_boxplot() + scale_fill_dutchmasters(palette="milkmaid") +                           # The type of plot that we will use
    facet_grid(Z~Alpha, labeller = label_both) + xlim(0,0.75) +                                 # Make several panels with specific limits
    ylab("Estimator") + xlab(Prf_M[i]) +                                                     # The labels of each axis
    theme(text = element_text(size = 20),
          axis.text.y=element_blank(),
          axis.ticks.y=element_blank())                                                      # General layout options: Used to adjust the size of the text
...
AvSel_hat %>% 
  ...
  ggplot(aes(x = M, y = as.numeric(Sel), fill = M)) + 
  geom_col() +
  facet_grid(Z~S) +
  coord_flip() +
...
ggsave("Plot_SelMean.png", path = fold_graphs, width = 50, height = 50, units = "cm")

# Trimmed Mean (10%)
# True Selection
TAvSel <- Sel[,,] %>% 
  abs() %>%                                                                 # Take the absolute value
  apply(2:3, function(x){mean(x, trim = tr)}) %>%                         # Average across period
  apply(1, function(x){mean(x, trim = tr)}) %>%                           # Average across draw
  cbind(paste0("Cor(YS) = ", seq(from = 0.2, to = 0.8, by = 0.2))) %>% 
  as.data.frame() %>% 
  rename("Sel" = 1 , "S" = 2)

# Estimated Selection
TempTAvSel_Z <- vector(mode = "list", length = n_Est)
TAvSel_hat <- data.frame(matrix(nrow = 0, ncol = 4))
for (j in 1:n_aux){
  for (i in 1:n_Est){
    TempTAvSel_Z[[i]] <- get(paste0("Sel_hat_Z",j))[[i]][,,] %>% 
      abs() %>%                        # Take the absolute value
      apply(2:3, function(x){mean(x, trim = tr, na.rm = TRUE)}) %>%                         # Average across period
      apply(1, function(x){mean(x, trim = tr, na.rm = TRUE)}) %>%                           # Average across draw
      cbind(paste0("Cor(YS) = ", seq(from = 0.2, to = 0.8, by = 0.2))) %>% 
      as.data.frame() %>% 
      rename("Sel" = 1 , "S" = 2) %>% 
      mutate(M = Est[i], Z = paste0("Cor(YZ) = ", cor_aux[j]))
  }
  TAvSel_hat <- rbind(TAvSel_hat, bind_rows(TempTAvSel_Z, .id = "column_label"))
}
...
TAvSel_hat %>% 
  ...
  facet_grid(Z~S) +
  coord_flip() +
  geom_hline(data =TAvSel, aes(yintercept=abs(as.numeric(Sel))), col = "blue", linetype = "dashed") +
  theme(strip.background = element_blank(), strip.placement = "outside", legend.position="none") +
  ylab("Selectivity") + xlab("Estimator")  + theme(text = element_text(size = 15)) +
  scale_fill_brewer(palette="Spectral")
ggsave("Plot_SelMeant.png", path = fold_graphs, width = 50, height = 50, units = "cm")
...
MedSel_hat %>% 
  ...
  ylab("Selectivity") + xlab("Estimator")  + theme(text = element_text(size = 15)) +
  scale_fill_brewer(palette="Spectral")
ggsave("Plot_SelMed.png", path = fold_graphs, width = 50, height = 50, units = "cm")
