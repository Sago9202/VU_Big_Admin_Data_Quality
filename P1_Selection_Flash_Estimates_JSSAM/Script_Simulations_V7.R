...
MUB_h <- function(Y, S, Z){
  ...
  r_yz_hat <- mapply(cor, as.data.frame(Y), as.data.frame(Z))
  phi <- (dy - r_yz_hat*dz)/((dy - r_yz_hat*dz) + (dz - r_yz_hat*dy))
  # Get the info from the lags
  Yl <- (cbind(rep(NA, times = nrow(Y)), Y[,-P]))
  Zl <- (cbind(rep(NA, times = nrow(Z)), Z[,-P]))
  Sl <- (cbind(rep(NA, times = nrow(S)), S[,-P]))
  ysl <- Yl*Sl; ysl <- ifelse(Sl==1, ysl, NA)
  zsl <- Zl*Sl; zsl <- ifelse(Sl==1, zsl, NA)
...
corS <- function(Y, rho, sample_rate = 0.5, var_strength = 0.05, noise_sd = 0.5){
  ...
  for (d in 1:D){
    for (p in 1:Time){
      ...
      X_signal <- rho*sd(Y.est)*Y_temp + Y.est*sd(Y_temp)*sqrt(1-rho^2)
      noise <- rnorm(length(Y_temp), mean = 0, sd = noise_sd)
      X_temp <- X_signal + noise
...
Est <- c("DDP_h", "MUB_h","DDP_a","MUB_a", "DDP_m", "MUB_m")
mres <- c("MAD","RMSD")
...
Est2 <- c("DDP(h)", "MUB(h)","DDP(a)","MUB(a)", "DDP(m)", "MUB(m)")
