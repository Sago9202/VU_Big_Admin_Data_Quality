...
MUB_f <- function(Y,S,Z){
  ...
  for (t in 1:t_total){
    ...
    z[,t][z[,t]==0] <- NA 
    # Elements of phi
    Delta_Y[t] <- (mean(y[,t], na.rm = TRUE) - mean(Y[,t]))/sd(Y[,t])
    Delta_Z[t] <- (mean(z[,t], na.rm = TRUE) - mean(Z[,t]))/sd(Z[,t])
...
MUB_ht <- function(Y,S,Z){
  ...
  res <- list(S_MUB, phi_hat, phi, Delta_Y, Delta_Z, rho_zy)
  return(res)
}
...
#### - (VIII) In which cases do we have phi outside of the boundary? - ####
  ...
  for (cs in 1:n_sel){
    for (cz in 1:n_aux){
        ...
        for(t in 1:Time){
          ...
          r_yz <- cor(Ye[,t], Ze[,t])
          r_ys <- cor(Ye[,t], Se[,t])
          mat_check[cs+((cz-1)*4),2+t] <- ((r_zs>=r_yz*r_ys & r_ys>=r_zs*r_yz) | (r_zs<=r_yz*r_ys & r_ys<=r_zs*r_yz))
        }
    }
  }
...
for (i in 1:30){
  print(check_phi[[i]]==check_cors[[i]])
}
