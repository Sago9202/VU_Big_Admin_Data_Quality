...
for (d in 1:D){
  for (c in 1:length(cor_sel)){
   ...
   Selh_Meng_rYS_hat[,c,d] <- as.vector(MAD(as.matrix(Y[,,d]), as.matrix(S[[c]][,,d]), Meng_rYS_hat[,c,d])[[2]])
  }
}
  ...
  for (m in 1:length(Measures)){
    ...
    for (d in 1:D){
     for (cs in 1:length(cor_sel)){
      ...
      Selh_Meas_hat[[m]][,cs,d] <- as.vector(MAD(as.matrix(Y[,,d]), as.matrix(S[[cs]][,,d]), Meas_hat[[m]][,cs,d])[[2]])
     }
    }
...
#saveRDS(Selh_Meas_hat, file = pasteO(fold_data, "/Sel_Dat.RData")) 
dat_sel <- readRDS(paste0(fold_code, "/Sel_Dat.RData"))  

for (i in 1:4){
  colnames(dat_sel[[i]]) <- c("t", "M", paste0("S",i))
  dat_sel[[i]] <- melt(dat_sel[[i]], id = c("t", "M"))
}

Dat_Sel <- rbind(dat_sel[[1]],dat_sel[[2]],dat_sel[[3]],dat_sel[[4]])
Dat_Sel_Av <- Dat_Sel %>%
  group_by(M, variable) %>%
  mutate(sel = mean(value, na.rm = TRUE)) %>%
  select(M, variable, sel)
Dat_Sel_Av$variable <- as.character(Dat_Sel_Av$variable)
Dat_Sel_Av$variable[Dat_Sel_Av$variable=="S1"] <- "Cov(YS) = 0.2"
Dat_Sel_Av$variable[Dat_Sel_Av$variable=="S2"] <- "Cov(YS) = 0.4"
Dat_Sel_Av$variable[Dat_Sel_Av$variable=="S3"] <- "Cov(YS) = 0.6"
Dat_Sel_Av$variable[Dat_Sel_Av$variable=="S4"] <- "Cov(YS) = 0.8"

ggplot(data = Dat_Sel_Av, aes(x = M, y = sel, fill = M)) +
  geom_col() +
  facet_grid(~variable) +
  coord_flip() +
